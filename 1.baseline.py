...
config["run_name"] = f"{config['optimizer']}_fc{config['fc_lr']}_bb{config['backbone_lr']}"
config["save_dir"] = f"outputs/{config['run_name']}"

def run_baseline(config, run_name=None):
    ...
    model = module.build_baseline_model(pretrained=True).to(device)

...
config["run_name"] = f"pretrained=False_{config['optimizer']}_bb{config['backbone_lr']}"
config["save_dir"] = f"outputs/{config['run_name']}"

def run_no_pretrained(config, run_name=None):
    ...
    model = module.build_baseline_model(pretrained=False).to(device)
...
if __name__ == "__main__":
    run_no_pretrained(config=config)